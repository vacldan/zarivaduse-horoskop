...
# Volání API s 429 backoff
def safe_call(endpoint, params, retries=3, delay=2):
    for i in range(retries):
        data = call_prokerala_api(endpoint, params)
        if data is not None:
            return data
        time.sleep(delay)
        delay *= 2
...
# Cache pro ukládání výsledků výpočtu (ttl 1h)
@st.cache_data(ttl=3600)
def fetch_data(endpoint, params):
    return safe_call(endpoint, params)
...
def call_prokerala_api(endpoint, params):
    ...
    try:
        ...
        # HTTP chyby vrací None, safe_call na to čeká
        return None
    except Exception as e:
        st.error(f"Chyba API: {e}")
        return None

# Validace a formát datetime
...
def create_planet_table(planets):
    ...
    pl = planets.get("planet_position") if isinstance(planets, dict) else planets
...
def display_horoscope_results(data):
    create_planet_table(data)
    create_svg_chart(data)
