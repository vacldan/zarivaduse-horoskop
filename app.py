...
# Elementy a barvy pro pozadí segmentů
element_colors = {
...
# Obalení logiky do try/except pro odchyt chyb
def main():
    try:
        # Funkce pro získání tokenu\ n        def get_access_token():
        ...
        # Volání API\ n        def call_prokerala_api(endpoint, params):
        ...
        def create_svg_chart(planets):
            ...
            if isinstance(planets, dict) and "planet_position" in planets:
                ...
                return
            size = 450; cx = cy = size/2
            r_outer = size * 0.45; r_inner = r_outer * 0.9
            ay = 23.9
            ...
            for p in pl:
                ...
                px = cx + r_inner * math.cos(ang); py = cy - r_inner * math.sin(ang)
        ...
        st.error("Došlo k neočekávané chybě při inicializaci aplikace.")
        st.text(str(e))
        st.text(traceback.format_exc())
        st.stop()
